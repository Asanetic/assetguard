...
export default function PendingapprovalProfile({ id: idProp, onDone }) {
  const searchParams = useSearchParams();
  const id = idProp ?? searchParams.get(`${PendingapprovalSchema.entity}_dataNode`);
  const form = useEntityFormController(PendingapprovalSchema, {
    id,
    onDone,
    redirectOnDelete: '/assetguard/Pendingapproval',
  });

  // TestGrid (devices at this site) intentionally left out for now:
  // <TestGrid schema={PendingapprovalSchema} fixedQuery={{ site_id: id }} title="Devices at this Site" />

  return (
    <DynamicForm
      controller={form}
      eyebrow={form.isEditing ? 'Pendingapproval Profile' : 'Pendingapproval Directory'}
      title={form.isEditing ? mosyGetSchemaTitle(PendingapprovalSchema, form.values, '') : 'New Pendingapproval'}
