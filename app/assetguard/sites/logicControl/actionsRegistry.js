...
export function handleCompanyData(item) {
  alert(`handleCompanyData -- ${item.company_name}`);
  return {
    location_address: item.company_address ?? '',
    // add more mappings any time — just return more keys
  };
}

export function handleManagerData(item) {
  //alert(`handleManagerData -- ${item}`);
  return {
    manager_mobile: item.tel ?? 'rr',
...
const SitesActions = {
  ...
  gridCheckBoxAction: async ({ rows }) => {
    ...
    console.log("gridCheckBoxAction --- ",rows);
