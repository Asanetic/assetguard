...
// DevicesProfile — pure shell. It resolves the id, wires up the
...
export default function DevicesProfile({ id: idProp, onDone }) {
  const searchParams = useSearchParams();
  const id = idProp ?? searchParams.get(`${DevicesSchema.entity}_dataNode`);
  const form = useEntityFormController(DevicesSchema, {
    id,
    onDone,
    redirectOnDelete: '/assetguard/Devices',
  });

  // TestGrid (devices at this site) intentionally left out for now:
  // <TestGrid schema={DevicesSchema} fixedQuery={{ site_id: id }} title="Devices at this Site" />

  return (
    <DynamicForm
      controller={form}
      eyebrow={form.isEditing ? 'Devices Profile' : 'Devices Directory'}
      title={form.isEditing ? mosyGetSchemaTitle(DevicesSchema, form.values, '') : 'New Devices'}
