...
import { AddDevicePings } from "../DevicePingsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("device_pings", node, req, newRecordId, authData);

     const insertResult = await AddDevicePings(newRecordId, mutatedObj, req, authData);
