...
import { DeleteDevicePings } from '../devicepings/DevicePingsDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_device_pings_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'device_pings'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteDevicePings(deleteTokenDecode, whereStr);
