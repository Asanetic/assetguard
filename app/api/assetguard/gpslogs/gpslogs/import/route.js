...
import { AddGpsLogs } from "../GpsLogsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("gps_logs", node, req, newRecordId, authData);

     const insertResult = await AddGpsLogs(newRecordId, mutatedObj, req, authData);
