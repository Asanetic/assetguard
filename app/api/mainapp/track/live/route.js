...
export async function GET(request) {
  ...
  try {
    const { rows } = await query(
      `SELECT COALESCE(t.device_time, t.received_at) AS at, t.lat, t.lng, t.speed
    ...
    const r = rows[0];
    const pos = r ? { lat: Number(r.lat), lng: Number(r.lng), speed: r.speed != null ? Math.round(Number(r.speed)) : null, at: r.at } : null;
